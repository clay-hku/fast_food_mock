...
dim_customer_zip="insert overwrite table dim_customer_zip partition (dt='9999-12-31')
select
    id,
    phone_number,
    username,
...
from
    (
        select
            ...
            '9999-12-31' end_date,
            row_number() over (partition by data.id order by data.update_time) rn
...
dim_product_group_full="insert overwrite table dim_product_group_full partition(dt = '2023-06-14')
select
    id             ,
    name           ,
    original_price ,
